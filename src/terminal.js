...
const TerminalInputWidget = require('./widgets/terminal_input')
...
const BuyCommand = require('./commands/orders/buy')
const ClearCommand = require('./commands/meta/clear')
const ExecuteCommand = require('./commands/meta/execute')
const ListExecutableCommand = require('./commands/meta/list-executable')
const SetSettingCommand = require('./commands/settings/set')
const GetSettingCommand = require('./commands/settings/get')
const ListSettingsCommand = require('./commands/settings/list')
...
// Can be aggressive, blessed only updates changed content. The update rate is
// only a problem during periods of exceptionally high trade volume.
...
class Terminal {
  constructor ({ symbol, apiKey, apiSecret }) {
    ...
    this.host.addModule(TerminalInputWidget, {
      geo: { x: 5, y: 12, w: 3, h: 2 }
    })
    ...
    this.host.addModule(SetSettingCommand)
    this.host.addModule(GetSettingCommand)
    this.host.addModule(ListSettingsCommand)
    this.host.addModule(ExecuteCommand)
    this.host.addModule(ListExecutableCommand)
    this.host.addModule(ClearCommand)
    this.host.addModule(BuyCommand)
  ...
  async connect () {
    ...
    */
    ...
    // TODO: Refactor out
    this.host.on('dataTradeEntry', ({ symbol, trade }) => {
    ...
    // TODO: Refactor out
    this.host.on('dataOrderBook', ({ orderBook }) => {
