...
class ModuleHost extends AsyncEventEmitter {
   ...
   *
   * @fires ModuleHost~hostBootStarted
   * @fires ModuleHost~hostBootFinished
   * @async
   */
  async boot () {
    if (this.isBooted()) {
    ...
    await (this.logDuration('module boot start', false)(async () => {
      /**
       * Primary init hook for modules
       *
       * @event ModuleHost~hostBootStarted
       */
      return this.emit('hostBootStarted')
    }))

    await (this.logDuration('module boot finish', false)(async () => {
      /**
       * Final init hook, at this point the modules are stable, so user input
       * can be enabled.
       *
       * @event ModuleHost~hostBootFinished
       */
      return this.emit('hostBootFinished')
    }))
 ...
 * @property {string} HOST_BOOT_STARTED - hook triggered once the module host
 *   is deemed complete, and execution may begin. Plugins should use this hook
 *   to start up.
 */
ModuleHost.HOST_PROVIDED_HOOKS = {
  HOST_ERROR: 'hostError',
  HOST_BOOT_STARTED: 'hostBootStarted',
  HOST_BOOT_FINISHED: 'hostBootFinished'
}
