...
class Terminal extends ModuleHost {
  constructor ({ symbol, apiKey, apiSecret }) {
    ...
    this.modules = []
    this.terminalRequiredModules = [
      'plugin:core:logger'
    ]

    this.terminalProvidedHooks = [
      'terminalBooted',
      'terminalError'
    ]

    this.addModule(new CoreLoggerPlugin(this))
    this.addModule(new CoreSettingsPlugin(this))
    this.addModule(new CoreNotificationsPlugin(this))
    this.addModule(new WSConnectionPlugin(this))
    this.addModule(new DataMarginInfoPlugin(this))
    this.addModule(new DataMarketInfoPlugin(this))
    this.addModule(new DataTradesPlugin(this))
    this.addModule(new DataPositionsPlugin(this))
    this.addModule(new DataOrderBooksPlugin(this))
    this.addModule(new DataOrdersPlugin(this))
    this.addModule(new DataCandlesPlugin(this))
    this.addModule(new MetaTradeGroupsPlugin(this))
    ...
    this.addModule(new StatusWidget(this, {
      geo: { x: 0, y: 5, w: 1, h: 6 }
    }))

    this.addModule(new ChartWidget(this, {
      tf: '1m',
      window: 180,
      emaPeriod: 50,
  ...
  /**
   * Verifies modules required by terminal are loaded, and emits/waits for the
   * event:Terminal~terminalBooted event to complete. Calls {@link
   * Terminal.destroyWithError} on failure to make the error visibile even
   * after blessed teardown.
   *
   * @fires Terminal~terminalError
   * @fires Terminal~terminalBooted
   */
  onBoot () {
    const moduleIDs = this.modules.map(m => m.getID())

    this.terminalRequiredModules.forEach((requiredID) => {
      if (!_includes(moduleIDs, requiredID)) {
        this.destroyWithError(
          new Error(`Terminal missing required module: ${requiredID}`)
        )
      }
    })

    // Ensure fallback error handler & terminal boot do not fail after all
    // modules are loaded, as blessed clears the screen on unhandled errors,
    // making debugging nearly impossible.

    /**
     * Low-level terminal error event, used when log methods are either not
     * available or cannot be relied upon (i.e. pre-boot)
     *
     * @event Terminal~terminalError
     * @property {Error} error - error object
     */
    this.emit('terminalError', {
      error: new ErrorHandlerSelfTestError()
    }).catch((e) => {
      this.destroyWithError(new Error(`terminalError failure: ${e.message}`))
    }).then(() => {
      return this.logDuration('module boot', false)(async () => {
        /**
         * Indicates module load completion, used as init hook for modules
         *
         * @event Terminal~terminalBooted
         */
        return this.emit('terminalBooted')
      })
    }).catch((e) => {
      this.destroyWithError(new Error(`terminalBooted failure: ${e.message}`))
    })
  }

  /**
   * Helper to log the duration of an operation in a standard way
   *
  ...
  logDuration (operationLabel, updateLogLine, ...measureDurationArgs) {
    this.logInfo('started: %s', operationLabel)
    ...
    return async (handler) => {
      ...
      this.logDebugClearLastLine()
      this.logInfo('done: %s %s', operationLabel, duration)
  ...
  addModule (module) {
    ...
    const allModules = [id, ...this.modules.map(m => m.getID())]

    requiredModules.forEach((requiredID) => {
      if (!_includes(allModules, requiredID)) {
        throw new ModuleAddError(
