...
const MarketInfoDataPlugin = require('../plugins/market_info_data')
const MarginInfoDataPlugin = require('../plugins/margin_info_data')
...
class Terminal extends EventEmitter {
  constructor ({ symbol, apiKey, apiSecret }) {
    ...
    this.addModule(new SettingsPlugin(this))
    this.addModule(new TradeGroupsPlugin(this))
    this.addModule(new NotificationsPlugin(this))
    this.addModule(new MarginInfoDataPlugin(this))
    this.addModule(new MarketInfoDataPlugin(this))
  ...
  async connect () {
    ...
    await this.ws.open()
    this.logSuccess('connected!')

    await this.ws.auth()
    this.logSuccess('authenticated!')
    ...
    this.logInfo('subscribing to trades for %s...', symbol)
    await this.ws.subscribeTrades(symbol)
    this.logSuccess('subscribed to trades for %s', symbol)

    this.logInfo('subscribing to 1min candles for %s...', symbol)
    await this.ws.subscribeCandles(candleKey)
    this.logSuccess('subscribed to candles for %s', symbol)

    this.logInfo('subscribing to order book for %s...', symbol)
    await this.ws.subscribeOrderBook(symbol, 'P0', '25')
    this.logSuccess('subscribed to order book for %s', symbol)
