...
const cancelCommand = (terminal, l) => y => defineCommandYargs(y, {
  ...
  options: {
    id: {
      ...
      choices: ['all', ...terminal.getActiveOrderIDs().map(id => `${id}`)]
    }
  },

  handler: async (argv) => {
    const { id, all } = argv
    const allIDs = terminal.getActiveOrderIDs()
    ...
    await terminal.cancelOrdersByID(idsToCancel)
