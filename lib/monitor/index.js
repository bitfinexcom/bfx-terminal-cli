...
class Monitor {
  constructor ({ apiKey, apiSecret }) {
    ...
    this.screenOrderLogBox = this.screenGrid.set(11, 0, 3, 4, blessed.element, {
      label: 'Order Log',
      ...SCROLLABLE_ELM_OPTIONS
    })

    this.screenConsoleOutputBox = this.screenGrid.set(2, 4, 11, 4, blessed.element, {
  ...
  updateStatus () {
    ...
    const tradableBalance = _isFinite(+marginNet)
      ? (marginNet * this.maxLeverage).toFixed(2)
      : '-'
    ...
    const statusContent = [
      ...
      '',
      `P/L: ${plCL(pl)}`,
      `Balance: ${marginBalance.green}`,
      `Net: ${marginNet.green}`,
      `Tradable: ${colors.bgMagenta.black(tradableBalance)}`,
      '',
